...
describe('testing SignIn Component', () => {
  ...
  const store = mockStore({}); // можем указать пустой объект в качестве аргумента, показывая, что нам не потребуется обращаться к хранилищу. В принципе это не обязательно, можно оставить просто пустые скобки
